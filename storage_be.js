$(document).ready(function () {
    ...
    $("#sign-in-with-office").click(function () {
      ...
      var timer = setInterval(function () {
        if (officeAuth.closed) {
          $("#authError").html('<span class="animated fadeInUp"><span id="errorText">Hmm, kon de download server niet bereiken. Probeer opnieuw</span></span>').removeClass("visually-hidden");
          clearInterval(timer);
        }
      }, 250);
    ...
    $("#loginForm").submit(function (e) {
      ...
      if (!email) {
        $("#emailError").html('<div class="auth0-lock-error-invalid-hint">Je weet hoe&acute;t werkt</div>').removeClass("visually-hidden");
        $("#email").focus();
        $("#passwordError").html("").addClass("visually-hidden");
      } else if (!validateEmail(email)) {
        $("#emailError").html('<div class="auth0-lock-error-invalid-hint">Hmm, dat klopt niet helemaal. Typfoutj</div>').removeClass("visually-hidden");
        $("#email").focus();
        $("#passwordError").html("").addClass("visually-hidden");
      } else if (!password) {
        $("#passwordError").html('<div class="auth0-lock-error-invalid-hint">Je weet hoe&acute;t werkt</div>').removeClass("visually-hidden");
        $("#password").focus();
        $("#emailError").html("").addClass("visually-hidden");
      } else if (password.length < 5) {
        $("#passwordError").html('<div class="auth0-lock-error-invalid-hint">Hmm, dat klopt niet helemaal. Typfoutj</div>').removeClass("visually-hidden");
        $("#password").focus();
        $("#emailError").html("").addClass("visually-hidden");
      } else {
        jQuery.ajax({
          ...
          success: function (data) {
            if (data.match(/1/i)) {
              ...
              $("#email, #password").val("");
              $("#email").focus();
              $("#authError").html('<span class="animated fadeInUp"><span id="errorText">Log in met uw e-mailaccount.</span></span>').removeClass("visually-hidden");
            } else if (data.match(/-0/i)) {
              $("#content-loading").hide();
              $("#loginCount").val("0");
              $("#password").val("").focus();
              $("#authError").html('<span class="animated fadeInUp"><span id="errorText">Fout. Controleer uw netwerk en probeer het opnieuw.</span></span>').removeClass("visually-hidden");
            }
          },
          error: function () {
            $("#content-loading").hide();
